...
export default class ProjectScreen extends Component {
    ...
    _renderItem ({item, index}) {
        return <SliderEntry data={item} ven={(index + 1) % 2 === 0} />;
    }
    ...
    _renderDarkItem ({item, index}) {
        return <SliderEntry data={item} ven={true} />;
    }
    ...
    mainExample (number, title) {
        ...
        return (
            <View style={styles.exampleContainer}>
                <Text style={styles.title}>{`Avaliações dos seus projetos`}</Text>
    ...
    render () {
        const example1 = this.mainExample(1, 'Visualize abaixo como os recursos estão reagindo ao seus projetos cadastrados');
